...
`testimonial-stat-row` — Stats strip ("214,000+ Enterprise Clients & Developers", "0+ Countries Served").- Horizontal row of 4 stat cells, each cell with a large number in {typography.heading-lg} {colors.ink} and a label below in {typography.body-sm} {colors.steel}.
...
- AI product tile illustrations are SVG-based; remain crisp at all breakpoints.## Overview
...
- Muted ({colors.- Avatar imagery in testimonials uses 1:1 aspect ratio with {rounded.full} masking.
